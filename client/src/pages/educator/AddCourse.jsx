...
import uniqid from 'uniqid';
import Quill from 'quill';
import 'quill/dist/quill.snow.css'; // ✅ Added missing Quill CSS
import { assets } from '../../assets/assets';
  ...
  const handleChapter = (action, chapterId) => {
    if (action === 'add') {
      ...
      if (title) {
        ...
        setChapter([...chapter, newChapter]); // ✅ Fixed typo
      }
    } else if (action === 'remove') {
      setChapter(chapter.filter((chapter) => chapter.chapterId !== chapterId)); // ✅ Fixed typo
    } else if (action === 'toggle') {
      setChapter(
        chapter.map((chapter) =>
